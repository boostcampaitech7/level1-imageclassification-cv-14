    ...
    def __init__(self, is_train=True):
        if is_train:
            ...
            self.transform = transforms.Compose([
                ...
                transforms.RandomHorizontalFlip(p=0.3),  # 30% 확률로 이미지를 수평 뒤집기
                transforms.RandomVerticalFlip(p=0.3),  # 30% 확률로 이미지를 수직 뒤집기
                transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1)),  # 이동, 회전, 스케일 변환
                transforms.ColorJitter(brightness=0.2, contrast=0.5, saturation=0.1, hue=0.1),  # 밝기, 대비, 채도, 색조 조정
