...
class SketchTransform:
    def __init__(self, is_train=True):
        if is_train:
            ...
            self.transform = transforms.Compose([
                ...
                transforms.RandomVerticalFlip(p=0.5),  # 50% 확률로 이미지를 수직 뒤집기
                transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1)),  # 이동, 회전, 스케일 변환
                transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.1, hue=0.1),  # 밝기, 대비, 채도, 색조 조정
                transforms.Lambda(lambda img: self.add_gaussian_noise(img)),  # 가우시안 노이즈 추가
                transforms.RandomPerspective(distortion_scale=0.2, p=0.5),  # 랜덤 시점 변환
    ...
    def enhance_lines(self, img):
        ...
        enhanced_img = cv2.addWeighted(np.array(img), 0.7, edges_colored, 0.3, 0)  # 원본과 에지
        # 원본 이미지에 에지 강조를 더하기 위해 RGB로 변환 후 합성
        edges_colored = cv2.cvtColor(edges, cv2.COLOR_GRAY2RGB)
    ...
    def __call__(self, image):
        ...
        # 이미지에 전처리 적용
        transformed_image = self.transform(image)  # 배치 차원 추가 제거
